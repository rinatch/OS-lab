...
void sort_rq(struct task_struct *p)
{
    printk(KERN_ERR "got into sort_rq!\n");
    prio_array_t *array = p->array;

    list_t* new_task_ptr = &p->run_list;
    list_t* head_task_ptr = array->queue + p->prio;//point to the head of list
    list_t* prev_task_ptr = (array->queue + p->prio)->prev;//point to the head's prev
    list_t* next_task_ptr = (array->queue + p->prio)->next;//point to first data structure of list
    struct task_struct* next_task;
    struct task_struct* prev_task;
    struct task_struct* new_task;

    printk(KERN_ERR "prio = %d\n",p->prio);
    printk(KERN_ERR "next_task_ptr = %p\n",next_task_ptr);
    printk(KERN_ERR "head_task_ptr = %p\n",head_task_ptr);
    printk(KERN_ERR "prev_task_ptr = %p\n",prev_task_ptr);
    printk(KERN_ERR "new_task_ptr = %p\n",new_task_ptr);

    next_task = list_entry(next_task_ptr,struct task_struct, run_list);
    prev_task = list_entry(prev_task_ptr,struct task_struct, run_list);
    new_task = list_entry(new_task_ptr,struct task_struct, run_list);

    printk(KERN_ERR "next_task pid = %d\n",next_task->pid);
    printk(KERN_ERR "prev_task pid = %d\n",prev_task->pid);
    printk(KERN_ERR "new_task pid = %d\n",new_task->pid);

    printk(KERN_ERR "next_task->invitations_num = %d\n",next_task->invitations_num);
    printk(KERN_ERR "prev_task->invitations_num = %d\n",prev_task->invitations_num);
    printk(KERN_ERR "new_task->invitations_num = %d\n",new_task->invitations_num);


    int flag = 1;
    printk(KERN_ERR "before while!\n");
    int i=0;
    prev_task_ptr = head_task_ptr;

    while(flag){//search for place

        next_task = list_entry(next_task_ptr,struct task_struct, run_list);
        printk(KERN_ERR "i = %d\n",i);

        if(prev_task_ptr == next_task_ptr){//empty list
            printk(KERN_ERR "prev_task_ptr == next_task_ptr\n");
            flag = 0;
        }else if( (p->invitations_num) > (next_task->invitations_num) ){//already found the correct place
            printk(KERN_ERR "p->invitations_num > next_task->invitations_num\n");
            flag = 0;
        }else{
            prev_task_ptr = next_task_ptr;
    ...
    printk(KERN_ERR "after while!\n");

    __list_add(new_task_ptr , prev_task_ptr , next_task_ptr);
    ...
    list_t* task_it;
    task_t* curr_task;
    list_for_each(task_it, head_task_ptr)
    {
        curr_task = list_entry(task_it,task_t, run_list);
        printk(KERN_ERR "task ptr is %p\n",curr_task);
        printk(KERN_ERR "task pid is %d\n",curr_task->pid);

    }
    printk(KERN_ERR "end sort!!\n");
    return;
...
///HW2 additions:
void invite_schedule(task_t* other_process)
{

    //basic case only todo: compare with current
    runqueue_t *rq = this_rq();
    dequeue_task(other_process,other_process->array);
    if(other_process->invitations_num == 1){
        printk(KERN_ERR "invitations == 1\n");
        other_process->policy = SCHED_FIFO;
        other_process->prio = 1;//first priority queue
    }
   // enqueue_task(other_process,rq->active);


   /*  bubbleSort_list(&((other_process->array)->queue + other_process->prio), other_process->prio);
//debug:
    printk(KERN_ERR "printing list of processes\n");
    list_t* it;
    list_for_each(it, &other_process->array->queue + other_process->prio)
    {
        {
            curr_inv_list = list_entry(it,struct task_struct, queue + other_process->prio);
            printk(KERN_ERR "pid: %d.\n",curr_inv_list->pid);
            printk(KERN_ERR "invitations_num: %d.\n",curr_inv_list->invitations_num);
        }
    }*/
...
void respond_schedule(task_t* other_process)
{
    //basic case only!!!!!!!
    runqueue_t *rq = this_rq();
    dequeue_task(other_process,other_process->array);
    if(other_process->invitations_num == 0){
        other_process->policy = SCHED_OTHER;
        other_process->prio = 0;//only priority for SCHED_OTHER
    }
    enqueue_task(other_process,rq->active);
    //bubbleSort_list(&other_process->array->queue + other_process->prio);
    schedule();
}
