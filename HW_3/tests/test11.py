...
def main():
    ...
    print("test 11 - using FORK - write 2 messages and than close the listening file ")
    pid = os.getpid()
    ...
    if fork2.isChild:
        f2 = os.open(DEVICE_PATH0, os.O_RDWR)
        fcntl.ioctl(f2, CONNECT, connect_address(0, 12))
        os.write(f2, 'hello from 24680')
        fork2.exit()
    fork2.wait()

    # os.close(f1)
    # f2 = os.open(DEVICE_PATH0, os.O_RDWR)
    # fcntl.ioctl(f2, LISTEN, 12)
    # try:
