...
int sys_procenet_respond(pid_t pid, int response) {
    ...
    {
        curr_invitation = list_entry(invitation_it,struct invitations_list_node, ptr);
...
//    printk(KERN_ERR "invitation_it ptr = %p\n", invitation_it);
    ...
    if (response == 1) {
        ...
        if (!find_task_by_pid(pid)) {
           ...
           // printk(KERN_ERR "made the dead friend node1\n");

            *(&current->friends_num) = *(&current->friends_num) + 1;
            //printk(KERN_ERR "made the dead friend node2\n");
    ...
    *(&current->invitations_num) = *(&current->invitations_num) - 1;
    kfree(new_friend);
    printk(KERN_ERR "respond end\n");

//    task_t *tmp_other_process = find_task_by_pid(pid);//todo:fix first other process
