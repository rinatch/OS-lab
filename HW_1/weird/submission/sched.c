...
void invite_schedule(task_t* other_process)
{
    ...
    if(other_process->invitations_num == 1){
        ...
        other_process->prio = 99;//first priority queue
    }
...
void swap(struct task_struct *a, struct task_struct *b)
{
...
void bubbleSort_list(list_t start_queue,int prio){
    int swapped;
    struct task_struct *ptr1;
    struct task_struct *lptr = NULL;
    list_t *processes_it;

    if(list_empty(&start_queue)){
        return;
    }
    ...
    do
    {
        ...
        list_for_each(processes_it, &start_queue)
        {
            ptr1 = list_entry(processes_it, struct task_struct, run_list);
            if (ptr1->invitations_num > ptr1->next_task->invitations_num)
            {
        ...
        lptr = ptr1;
        /*ptr1 = (struct task_struct *)start_queue; //first data structure todo: maybe next

        while (ptr1->next != lptr)
        {
            if (ptr1->invitations_num > ptr1->next->invitations_num)
            {
                swap(ptr1, ptr1->next);
                swapped = 1;
            }
